...
import { AppointmentSelectOption } from '../../appointments/utils/appointment-picker.util';
import { getControlError } from '../../../shared/utils/form-errors.util';

@Component({
  selector: 'app-vital-entry-form',
  standalone: true,
  imports: [ReactiveFormsModule, FormFieldComponent, UiInputComponent, UiSelectComponent, UiTextareaComponent],
  template: `
    <div class="grid gap-4 sm:grid-cols-2" [formGroup]="form()">
      <app-form-field label="Appointment" class="sm:col-span-2" [error]="err('appointmentId')">
        @if (appointmentOptions().length > 0) {
          <app-ui-select formControlName="appointmentId" [options]="appointmentOptions()" placeholder="Select visit" />
        } @else {
...
export class VitalEntryFormComponent {
  ...
  readonly appointmentOptions = input<AppointmentSelectOption[]>([]);
