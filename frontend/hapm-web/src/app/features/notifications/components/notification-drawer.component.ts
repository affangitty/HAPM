...
@Component({
  ...
  template: `
    @if (open()) {
      ...
      <aside class="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l bg-card shadow-xl">
        <div class="flex items-center justify-between border-b px-4 py-3">
          <div>
            <h2 class="font-semibold">Notifications</h2>
            <p class="text-xs text-muted-foreground">{{ unreadCount() }} unread</p>
          </div>
          <button type="button" class="rounded-lg p-2 hover:bg-muted" (click)="close.emit()">✕</button>
        </div>
...
export class NotificationDrawerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.refresh();
    this.hub.received$.subscribe(() => this.refresh());
  }
